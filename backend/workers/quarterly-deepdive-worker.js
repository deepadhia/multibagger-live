    ...
    try {
      ...
      if (!suppressTelegram) {
        ...
        if (verdict.financial_highlights) {
          ...
          const parts = [];
          if (fh.revenue && fh.revenue !== "N/A") parts.push(`• *Revenue:* ${fh.revenue} (${fh.revenue_yoy || 'N/A'})`);
          if (fh.ebitda && fh.ebitda !== "N/A") parts.push(`• *EBITDA:* ${fh.ebitda} (Margin: ${fh.ebitda_margin || 'N/A'}, ${fh.ebitda_margin_delta || 'N/A'})`);
          if (fh.pat_consolidated && fh.pat_consolidated !== "N/A") parts.push(`• *Consolidated PAT:* ${fh.pat_consolidated} (${fh.pat_yoy || 'N/A'})`);
        ...
        if (concallPoints && Array.isArray(concallPoints) && concallPoints.length > 0) {
          ...
          concallText = `\n*🎙️ Concall Verification Points (What to check in Q&A):*\n• Re-affirmation of FY27 revenue & margin guidance\n• Execution timeline & order book delivery schedule\n• Working capital & Discom receivable collection trend\n`;
        }
