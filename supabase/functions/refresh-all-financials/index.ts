...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Refreshing financials for ${stocks.length} stocks`);
    ...
    for (const stock of stocks) {
      try {
        ...
        const resp = await fetch(`${SUPABASE_URL}/functions/v1/fetch-financials`, {
          method: "POST",
          headers: {
        ...
        if (data.error) {
          console.error(`Error for ${stock.ticker}:`, data.error);
          results.push({ ticker: stock.ticker, success: false, error: data.error });
        } else {
          console.log(`✓ ${stock.ticker} updated`);
    ...
    return new Response(JSON.stringify({
      message: `Refreshed ${successCount}/${stocks.length} stocks`,
      results,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
    console.error("refresh-all-financials error:", e);
