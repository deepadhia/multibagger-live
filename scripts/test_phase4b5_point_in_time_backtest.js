...
async function runPhase4B5_1DataIntegrityAudit() {
  ...
  if (process.env.ARTIFACTS_DIR && fs.existsSync(process.env.ARTIFACTS_DIR)) {
    reportPath = path.join(process.env.ARTIFACTS_DIR, "PHASE_4B5_1_OUTCOME_DATA_INTEGRITY_REPORT.md");
  } else if (fs.existsSync(brainDir)) {
    reportPath = path.join(brainDir, "PHASE_4B5_1_OUTCOME_DATA_INTEGRITY_REPORT.md");
  } else {
    ...
    reportPath = path.join(localArtifacts, "PHASE_4B5_1_OUTCOME_DATA_INTEGRITY_REPORT.md");
  }
