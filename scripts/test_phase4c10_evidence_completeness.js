...
async function runPhase4C10EvidenceCompletenessSuite() {
  ...
  if (process.env.ARTIFACTS_DIR && fs.existsSync(process.env.ARTIFACTS_DIR)) {
    reportPath = path.join(process.env.ARTIFACTS_DIR, "PHASE_4C10_MANAGEMENT_EVIDENCE_COMPLETENESS_REPORT.md");
  } else if (fs.existsSync(brainDir)) {
    reportPath = path.join(brainDir, "PHASE_4C10_MANAGEMENT_EVIDENCE_COMPLETENESS_REPORT.md");
  } else {
    ...
    reportPath = path.join(localArtifacts, "PHASE_4C10_MANAGEMENT_EVIDENCE_COMPLETENESS_REPORT.md");
  }
