...
async function runPhase4E5PortfolioAudit() {
  ...
  if (process.env.ARTIFACTS_DIR && fs.existsSync(process.env.ARTIFACTS_DIR)) {
    reportPath = path.join(process.env.ARTIFACTS_DIR, "PHASE_4E5_PORTFOLIO_AUDIT_REPORT.md");
  } else if (fs.existsSync(brainDir)) {
    reportPath = path.join(brainDir, "PHASE_4E5_PORTFOLIO_AUDIT_REPORT.md");
  } else {
    ...
    reportPath = path.join(localArtifacts, "PHASE_4E5_PORTFOLIO_AUDIT_REPORT.md");
  }
